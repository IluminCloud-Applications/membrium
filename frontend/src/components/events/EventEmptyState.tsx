...
export function EventEmptyState({ hasFilters, onCreateItem }: EventEmptyStateProps) {
    return (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground rounded-xl border bg-card shadow-sm">
            ...
            <h3 className="font-semibold text-foreground mb-1">
                {hasFilters ? "Nenhuma evento encontrada" : "Nenhuma evento criada"}
            </h3>

            <p className="text-sm mb-4 max-w-sm text-center">
                {hasFilters
                    ? "Tente ajustar os filtros ou a busca para encontrar a evento."
                    : "Comece criando sua primeira evento para engajar seus alunos!"}
            </p>

            {!hasFilters && (
                <Button onClick={onCreateItem} className="btn-brand text-sm">
                    <i className="ri-add-line mr-1" />
                    Criar Primeira Evento
                </Button>
            )}
        </div>
    );
