...
interface TranscriptLessonTableProps {
    items: Transcript[];
...
/**
 * Level 3: Shows lessons with transcripts within a module.
 * Each row has actions to view, edit, or delete.
 */
export function TranscriptLessonTable({
    items,
    onView,
    onEdit,
    onDelete,
}: TranscriptLessonTableProps) {
    return (
...
interface TranscriptLessonRowProps {
    item: Transcript;
    onView: (item: Transcript) => void;
    onEdit: (item: Transcript) => void;
    onDelete: (item: Transcript) => void;
...
function TranscriptLessonRow({ item, onView, onEdit, onDelete }: TranscriptLessonRowProps) {
    const maxKeywords = 3;
    ...
    return (
        <TableRow className="group cursor-pointer" onClick={() => onView(item)}>
            ...
            <TableCell className="px-4 text-center">
                ...
                >
                    {item.wordCount.toLocaleString("pt-BR")}
                </Badge>
            </TableCell>
